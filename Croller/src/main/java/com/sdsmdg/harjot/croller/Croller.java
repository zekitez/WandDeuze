...
public class Croller extends View {
    ...
    private int progressSecondaryColorMaxScale = Color.parseColor("#222222");
    ...
    private void initXMLAttrs(Context context, AttributeSet attrs) {
        ...
        setProgressSecondaryColorMaxScale(a.getColor(R.styleable.Croller_progress_secondary_color_max_scale, progressSecondaryColorMaxScale));
        ...
        setMaxScale(a.getInt(R.styleable.Croller_max_scale, 25));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!isContinuous) {
            ...
            for (int i = (int) (deg2); i < maxScale + 3; i++) {
                float tmp = ((float) startOffset2 / 360) + ((float) sweepAngle / 360) * (float) i / (maxScale + 5);
                ...
                if (i > max+2){
                    circlePaint.setColor(progressSecondaryColorMaxScale);
                }
                if (progressSecondaryCircleSize == -1) {
                    canvas.drawCircle(x, y, ((float) radius / 30 * ((float) 20 / maxScale) * ((float) sweepAngle / 270)), circlePaint);
                } else {
            ...
            for (int i = 3; i <= deg3; i++) {
                ...
                if (progressPrimaryCircleSize == -1) {
                    canvas.drawCircle(x, y, (progressRadius / 15 * ((float) 20 / maxScale) * ((float) sweepAngle / 270)), circlePaint2);
                } else {
                    canvas.drawCircle(x, y, progressPrimaryCircleSize, circlePaint2);
                }
            }

            float tmp2 = ((float) startOffset2 / 360) + ((float) sweepAngle / 360) * deg / (maxScale + 5);

            if (isAntiClockwise) {
                tmp2 = 1.0f - tmp2;
            }

            // Coordinates for indicator
            float x1 = midx + (float) (radius * (0.45) * Math.sin(2.0 * Math.PI * (1.0 - tmp2)));
            float y1 = midy + (float) (radius * (0.45) * Math.cos(2.0 * Math.PI * (1.0 - tmp2)));
            float x2 = midx + (float) (radius * (0.74) * Math.sin(2.0 * Math.PI * (1.0 - tmp2)));
            float y2 = midy + (float) (radius * (0.74) * Math.cos(2.0 * Math.PI * (1.0 - tmp2)));
            ...
            circlePaint.setColor( isEnabled ? mainCircleColor : mainCircleDisabledColor );
            ...
            canvas.drawText(label , midx, midy + textPaint.getFontMetrics().top, textPaint);
            canvas.drawText("Max: "+max, midx, midy , textPaint);
    ...
    public void setProgressSecondaryColorMaxScale(int progressSecondaryColorMaxScale) {
        this.progressSecondaryColorMaxScale = progressSecondaryColorMaxScale;
        invalidate();
    }
